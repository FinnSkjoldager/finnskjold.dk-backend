...
<div class="container mt-5">
    ...
    <select name="region" id="region-select" onChange="clickFunk(this)">
        <option value="dk1" @if($region == "dk1") selected @endif>Jylland og Fyn</option>
        <option value="dk2" @if($region == "dk2") selected @endif>Sjælland</option>
    </select>
        <table class="table table-inverse table-striped">
            ...
            <tbody>
                @foreach($p as $data)
                <tr>
                    <td>{{$data['PriceArea']}}</td>
                    <td>{{$data['HourDK']}}</td>
                    <td>{{number_format(($data['SpotPriceDKK'] / 1000),2 )}}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
